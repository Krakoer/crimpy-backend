...
-- Stores coach-created session templates (structured training sessions).
...
-- Stores individual items within a coach session (exercises, hangboards, circuits, sections).
...
CREATE TABLE "coach_session_items" (
  ...
  "parent_id"            UUID        REFERENCES "coach_session_items"("id") ON DELETE CASCADE,
  "type"                 TEXT        NOT NULL,
